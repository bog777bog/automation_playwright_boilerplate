...
test.describe('API tests', () => {
    ...
    test.only('validate GET single product', { tag: '@api' },  async ({ request }) => {
        const response = await request.get(Products.getSingleProductEndpoint('1'));
    ...
    test.only('validate GET search product', { tag: '@api' },  async ({ request }) => {
        const response = await request.get(Products.searchProductEndpoint('phone'));
        const data = await response.json();
