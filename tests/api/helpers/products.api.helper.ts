...
export class ProductsAPI { 
    ...
    async searchProducts(query: string) {
        const response = await this.request.get(Products.searchProductEndpoint(query));
    ...
    async getProduct (productId: string) {
        const response = await this.request.get(Products.getSingleProductEndpoint(productId));
        const data = await response.json();
